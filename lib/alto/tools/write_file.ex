defmodule Alto.Tools.WriteFile do
  ...
  defp preview(content, limit) do
    %{content: binary_part(content, 0, limit), truncated: true}
  end
  ...
  defp validate_options(opts) when is_list(opts) do
    if Keyword.keyword?(opts) do
      case NimbleOptions.validate(opts, @options_schema) do
        ...
        {:error, reason} -> {:error, {:invalid_write_options, reason}}
      end
    else
      {:error, {:invalid_write_options, opts}}
    end
  end

  defp validate_options(opts), do: {:error, {:invalid_write_options, opts}}
